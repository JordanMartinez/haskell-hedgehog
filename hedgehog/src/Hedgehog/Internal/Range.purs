...
-- Jordan's note: not sure why this is here...
-- $setup
...
derive newtype instance eqSize :: Eq Size
derive newtype instance ordSize :: Ord Size
derive newtype instance enumSize :: Enum Size

instance showSize :: Show Size where
  show (Size x) = "Size(" <> show x <> ")"

-- | Returns either the Size or the original string if it could not be parsed
-- | ```
-- | parseSize "Size4" == Right (Size 4)
-- | parseSize "size4" == Left "size4"
-- | ```
parseSize :: String -> Either String Size
parseSize s = case splitAt 4 s of
  {before: "Size", after } ->
...
-- Jordan's note: use `parseSize` instead of Read instance, which
-- doesn't exist in PureScript.
...
data Range a = Range a (Size -> Tuple a a)

instance functorRange :: Functor Range where
  fmap f (Range z g) =
    Range (f z) \sz ->
      bimap f f (g sz)
...
singleton x = Range x \_ -> Tuple x x
...
constantFrom :: forall a.
  ...
  -> Range a
constantFrom z x y = Range z \_ -> Tuple x y
...
-- | (-10,20)
linearFrom :: forall a. Integral a
  => a -- ^ Origin (the value produced when the size parameter is 0).
  -> a -- ^ Lower bound (the bottom of the range when the size parameter is 99).
  -> a -- ^ Upper bound (the top of the range when the size parameter is 99).
  -> Range a
linearFrom z x y =
  Range z \sz ->
    let
      x_sized = clamp x y $ scaleLinear sz z x
      y_sized = clamp x y $ scaleLinear sz z y
    in
      Tuple x_sized y_sized
...
linearBounded = linearFrom 0 minBound maxBound
...
linearFrac x y = linearFracFrom x x y
...
linearFracFrom z x y =
  Range z $ \sz ->
    let
      x_sized = clamp x y $ scaleLinearFrac sz z x
...
scaleLinear sz0 z0 n0 =
  let
    ...
    diff = ((n - z) * fromIntegral sz) `quot` 99
  in
    fromInteger $ z + diff
...
exponential x y = exponentialFrom x x y
...
-- | (-128,512)
exponentialFrom :: forall a. Integral a
  => a -- ^ Origin (the value produced when the size parameter is 0).
  -> a -- ^ Lower bound (the bottom of the range when the size parameter is 99).
  -> a -- ^ Upper bound (the top of the range when the size parameter is 99).
  -> Range a
exponentialFrom z x y =
  Range z $ \sz ->
    let
      sized_x = clamp x y $ scaleExponential sz z x
      sized_y = clamp x y $ scaleExponential sz z y
    in
      Tuple sized_x sized_y
...
exponentialBounded = exponentialFrom 0 minBound maxBound
...
exponentialFloat x y = exponentialFloatFrom x x y
...
exponentialFloatFrom z x y =
  Range z \sz ->
    let
      sized_x = clamp x y $ scaleExponentialFloat sz z x
